...
This document lists the assumptions made while designing and building the NovaBank cloud proof-of-concept. Where an assumption materially affects risk, cost, or architecture, the corresponding trade-off is also referenced in [`architecture-summary.md`](./architecture-summary.md).
...
- NovaBank leadership accepts the small/burstable SKUs chosen (App Service `P0v3`, PostgreSQL `B_Standard_B1ms` dev / `GP_Standard_D2s_v3` prod) as adequate for current load, understanding that vertical tier upgrades are the primary scaling lever.
